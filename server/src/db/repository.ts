...
export class BookmarkRepository {
  ...
  // Node operations
  async createFolder(
    nodeData: Omit<NewNode, 'kind'>,
    folderData: NewFolder,
  ): Promise<FolderWithNode> {
    return await db.transaction((tx) => {
      const nodeResult = tx
        .insert(nodes)
        .values({ ...nodeData, kind: 'folder' })
        .returning()
        .get();
      const folderResult = tx
        .insert(folders)
        .values({ ...folderData, nodeId: nodeResult.id })
        .returning()
        .get();
      return {
        ...nodeResult,
  ...
  ): Promise<BookmarkWithNode> {
    return await db.transaction((tx) => {
      const nodeResult = tx
        .insert(nodes)
        .values({ ...nodeData, kind: 'bookmark' })
        .returning()
        .get();
      const bookmarkResult = tx
        .insert(bookmarks)
        .values({ ...bookmarkData, nodeId: nodeResult.id })
        .returning()
        .get();
      return {
        ...nodeResult,
  ...
  async updateFolder(
    id: string,
    nodeUpdates: Partial<NewNode>,
    folderUpdates: Partial<NewFolder>,
  ): Promise<FolderWithNode | null> {
    return await db.transaction((tx) => {
  ...
  async updateBookmark(
    id: string,
    nodeUpdates: Partial<NewNode>,
    bookmarkUpdates: Partial<NewBookmark>,
  ): Promise<BookmarkWithNode | null> {
    return await db.transaction((tx) => {
      if (Object.keys(nodeUpdates).length > 0) {
        tx.update(nodes)
          .set({ ...nodeUpdates, updatedAt: new Date() })
  ...
  // Utility methods
  async buildSerializedTree(
    namespace?: string,
  ): Promise<{ rootId: string; nodes: Record<string, any> } | null> {
    const allNodes = namespace
      ? await this.getAllNodesByNamespace(namespace)
      : await this.getAllNodes();
    const rootNode = namespace
      ? await this.getRootNodeByNamespace(namespace)
  ...
  async initializeWithSampleData(): Promise<void> {
    ...
    const rootFolder = await this.createFolder(
      { id: 'root', parentId: null, namespace, createdAt: now, updatedAt: now, orderKey: '0' },
      { nodeId: 'root', title: 'Bookmarks', isOpen: true },
    );
    const devFolder = await this.createFolder(
      ...
      { nodeId: 'folder-1', title: 'Development', isOpen: true },
    );
    const designFolder = await this.createFolder(
      ...
      { nodeId: 'folder-2', title: 'Design', isOpen: false },
    );
    const githubBookmark = await this.createBookmark(
      ...
      { nodeId: 'bookmark-1', title: 'GitHub', url: 'https://github.com' },
    );
    const mdnBookmark = await this.createBookmark(
      ...
      { nodeId: 'bookmark-2', title: 'MDN Web Docs', url: 'https://developer.mozilla.org' },
    );
    const stackoverflowBookmark = await this.createBookmark(
      ...
      { nodeId: 'bookmark-3', title: 'Stack Overflow', url: 'https://stackoverflow.com' },
    );
    const figmaBookmark = await this.createBookmark(
      ...
      { nodeId: 'bookmark-4', title: 'Figma', url: 'https://figma.com' },
    );
    const sampleNodes = [
      rootFolder,
      devFolder,
