 ...
 */
export function convertServerEventToOperation(
  event: ServerEvent,
): TreeOperation | null {
  ...
  try {
    ...
    if (
      ...
      typeof data === "object" &&
      data &&
      "op" in (data as OperationEventData)
    ) {
    ...
    console.error(
      `[SSE Converter] Failed to convert event ${type}:`,
      error,
      data,
    );
    return null;
 ...
 */
export function convertServerEventToEnvelope(
  event: ServerEvent,
  ...
  if (
    ...
    event.data &&
    "op" in (event.data as OperationEventData)
  ) {
...
export function validateServerEventData(event: ServerEvent): boolean {
  ...
  try {
    ...
    if (type === "operation") {
      ...
      const hasTs =
        typeof od?.ts === "number" || typeof event.timestamp === "string";
      if (!hasOp)
        console.warn(`[SSE Converter] Missing 'op.type' in operation event`);
    ...
    for (const field of commonFields) {
      if (!(field in data)) {
        console.warn(
          `[SSE Converter] Missing required field '${field}' in ${type} event`,
        );
        return false;
      }
    }
